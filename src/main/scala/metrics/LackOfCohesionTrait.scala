...
trait LackOfCohesionTrait extends Metric {
  override def evaluate(source: String): Double = {
    ...
    tree.traverse {
      case cls: Defn.Class =>
        ...
        totalScore += score(empty, nonEmpty)
    }
    totalScore / classCount.toDouble
  }
  protected def score(empty: Int, nonEmpty: Int): Double
}
